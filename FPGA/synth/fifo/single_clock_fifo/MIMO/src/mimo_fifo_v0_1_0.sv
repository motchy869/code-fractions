...
module mimo_fifo_v0_1_0 #(
    ...
    input uwire logic i_clk, //! input clock
    input uwire logic i_sync_rst, //! input reset signal synchronous to the input clock
    input uwire logic i_freeze, //! freeze directive, which stops all state transitions except for the reset
    //! @end
    ...
    output uwire logic o_full,
    input uwire logic [$clog2(MAX_N_I+1)-1:0] i_n_i, //! Number of input elements. 0 corresponds to write-enable signal is low. This value is **clipped** to ```MAX_N_I```.
    input uwire T_E [MAX_N_I-1:0] i_elems, //! Input elements from upstream side. Only first ```i_n_i``` elements are valid.
    //! @end
    ...
    output uwire logic o_empty,
    input uwire logic [$clog2(MAX_N_O+1)-1:0] i_n_o, //! Number of required elements from downstream side. 0 corresponds to read-enable signal is low. This value is **clipped** to ```MAX_N_O```.
    output uwire T_E [MAX_N_O-1:0] o_elems //! Output elements to downstream side. Only first ```i_n_o``` elements are valid.
    //! @end
);
// ---------- parameters ----------
...
uwire [$clog2(N_SLTS+1)-1:0] g_free_elem_slts = $clog2(N_SLTS+1)'(N_SLTS) - r_elem_cnt; //! the number of free element-slots
...
uwire [$clog2(MAX_N_I+1)-1:0] g_n_i_clp = (i_n_i > $clog2(MAX_N_I+1)'(MAX_N_I)) ? $clog2(MAX_N_I+1)'(MAX_N_I) : i_n_i; //! the number of input elements, clipped to ```MAX_N_I```
uwire [$clog2(MAX_N_O+1)-1:0] g_n_o_clp = (i_n_o > $clog2(MAX_N_O+1)'(MAX_N_O)) ? $clog2(MAX_N_O+1)'(MAX_N_O) : i_n_o; //! the number of output elements, clipped to ```MAX_N_O```

uwire g_can_push = $clog2(N_SLTS+1)'(g_n_i_clp) <= g_free_elem_slts; //! indicates that elements can be pushed
uwire g_can_pop = $clog2(N_SLTS+1)'(g_n_o_clp) <= r_elem_cnt; //! indicates that elements can be popped
uwire g_push_now = (g_n_i_clp > '0) && g_can_push; //! indicates that the elements from upstream side should be pushed now
uwire g_pop_now = (g_n_o_clp > '0) && g_can_pop; //! indicates that the elements should be popped now
// --------------------
