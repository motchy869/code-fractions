...
`ifndef POW2_BST_OUT_RING_BUF_V0_1_0_SV_INCLUDED
`define POW2_BST_OUT_RING_BUF_V0_1_0_SV_INCLUDED

`default_nettype none

//! A ring buffer with power-of-two capacity and burst-output.
//!
//! This module receives **up to** ```2**EXP_OUT``` elements from the upstream side at one clock cycle, and outputs **exactly** ```2**EXP_OUT``` elements at one clock cycle.
//! There is **no handshake** function, so a parent module must handle the flow control.
module pow2_bst_out_ring_buf_v0_1_0 #(
    parameter int unsigned EXP_BUF = 7, //! Buffer size exponent, must be in range [1,31]. The buffer size is ```2**EXP_BUF```.
    parameter int unsigned EXP_OUT = 6, //! Output burst size exponent, must be in range [1,```EXP_BUF-1```]. The output burst size is ```2**EXP_OUT```.
    ...
    input wire logic [EXP_OUT:0] i_n_in, //! Number of input elements. When the condition ```i_n_in > 0 && i_n_in <= o_free_cnt``` holds,  at the next clock rising-edge, ```i_n_in``` elements are stored in the buffer.
    `ifndef COMPILER_MATURITY_LEVEL_0
        input wire T_ELEM [2**EXP_OUT-1:0] i_in_elems, //! Input elements. Only first ```i_n_in``` elements are valid.
    `else
        input wire logic [2**EXP_OUT-1:0][BW_ELEM-1:0] i_in_elems, //! Input elements. Only first ```i_n_in``` elements are valid.
    `endif
    //! @end
    ...
    `endif
    input wire logic i_ds_ready //! Ready signal indicating that the output elements can be accepted by the downstream side. When the condition ```o_cnt >= 2**EXP_OUT && i_ds_ready``` holds, at the next clock rising-edge, ```2**EXP_OUT``` elements are removed from the buffer.
    //! @end
);
// ---------- imports ----------
// --------------------
...
wire logic g_str_en = i_n_in > '0 && BW_BUF_CNT'(i_n_in) <= r_free_cnt; //! store-enable signal
wire logic g_cns_en = r_cnt >= BW_BUF_CNT'(OUT_BST_SIZE) && i_ds_ready; //! consume-enable signal
// --------------------
...
generate
    for (i_gen=0; i_gen<BUF_CAPACITY; i_gen++) begin: gen_drv_o_mrr_buf
        //assign o_mrr_buf[i_gen] = r_buf[r_head_idx + i_gen]; // This fails. Somehow ```o_mrr_buf[15:0]``` becomes indefinite.
        assign o_mrr_buf[i_gen] = r_buf[r_head_idx + unsigned'(BW_BUF_IDX'(i_gen))];
    end
endgenerate
// --------------------
...
always_ff @(posedge i_clk) begin: blk_update_cnt_and_free
    if (i_sync_rst) begin
        ...
        automatic logic [BW_BUF_CNT-1:0] n_str = g_str_en ? BW_BUF_CNT'(i_n_in) : '0;
        automatic logic [BW_BUF_CNT-1:0] n_cns = g_cns_en ? BW_BUF_CNT'(OUT_BST_SIZE) : '0;
        r_cnt <= r_cnt + n_str - n_cns;
        r_free_cnt <= r_free_cnt - n_str + n_cns;
    end
end
...
always_ff @(posedge i_clk) begin: blk_update_idx
    if (i_sync_rst) begin
        ...
        r_head_idx <= r_head_idx + (g_cns_en ? BW_BUF_IDX'(OUT_BST_SIZE) : '0);
        r_tail_idx <= r_tail_idx + (g_str_en ? BW_BUF_IDX'(i_n_in) : '0);
    end
end
...
always_ff @(posedge i_clk) begin: blk_update_buf
    if (i_sync_rst) begin
        ...
        automatic logic [BW_BUF_IDX-1:0] str_end_idx = r_tail_idx + BW_BUF_IDX'(i_n_in-1);
...
`endif // POW2_BST_OUT_RING_BUF_V0_1_0_SV_INCLUDED
