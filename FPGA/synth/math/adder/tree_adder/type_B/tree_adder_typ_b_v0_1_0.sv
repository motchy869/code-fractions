...
function automatic adder_plan_t planAdders();
    ...
    for (int unsigned i=0; i<n_adders; ++i) begin
        ...
        plan.min_val_mat[0][i] = MIN_VALS[2*i] + MIN_VALS[2*i+1];
        plan.max_val_mat[0][i] = MAX_VALS[2*i] + MAX_VALS[2*i+1];
    end
    if (N_IN_ELEMS[0] == 1'b1) begin
        plan.min_val_mat[0][n_adders] = MIN_VALS[2*n_adders];
        plan.max_val_mat[0][n_adders] = MAX_VALS[2*n_adders];
    end
    ...
    for (int unsigned d=0; d<TREE_HEIGHT; ++d) begin
        for (int unsigned i=0; i<plan.n_elems[d]; ++i) begin
            min_max_t min = plan.min_val_mat[d][i], max = plan.max_val_mat[d][i];
            int unsigned bw_sum_min = min >= '0 ? BW_OUT_PORT - cntLdgZeros(min) + 1 : BW_OUT_PORT - cntLdgOnes(min) + 1;
            int unsigned bw_sum_max = max >= '0 ? BW_OUT_PORT - cntLdgZeros(max) + 1 : BW_OUT_PORT - cntLdgOnes(max) + 1;
            plan.bwMat[d][i] = bw_sum_min <= bw_sum_max ? bw_sum_max : bw_sum_min;
        end
    end
