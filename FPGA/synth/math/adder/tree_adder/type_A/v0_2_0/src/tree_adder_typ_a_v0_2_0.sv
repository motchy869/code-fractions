...
module tree_adder_typ_a_v0_2_0 #(
    ...
    parameter bit USE_DSP_RST = 1'b1, //! Whether to clear the DSP-related numerical data at reset. If not, numerical data registers are not cleared and P&R feasibility will be improved.
    parameter bit USE_CLK_EN = 1'b1 //! Whether to use clock enable signal. If not, the port exists but not used, and P&R feasibility will be improved.
)(
...
generate
    if (N_IN_ELEMS < 2) begin: gen_too_small_N_IN_ELEMS
        nonexistent_module_to_throw_an_error_message_for_too_small_N_IN_ELEMS inst();
    end
    if (BW_IN_ELEM < 1) begin: gen_too_small_BW_IN_ELEM
        nonexistent_module_to_throw_an_error_message_for_too_small_BW_IN_ELEM inst();
    end
endgenerate
// --------------------
...
always_ff @(posedge i_clk) begin: blk_update_vld_dly_line
    if (i_sync_rst) begin
        r_vld_dly_line <= '0;
    end else if (!USE_CLK_EN || i_clk_en) begin
