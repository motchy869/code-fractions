...
generate
    if (N_F < 1) begin: gen_validate_n_f_lower_bound
        nonexistent_module_to_throw_a_custom_error_message_for_validate_n_f_lower_bound inst();
    end

    if (N_F >= N) begin: gen_validate_n_f_upper_bound
        nonexistent_module_to_throw_a_custom_error_message_for_validate_n_f_upper_bound inst();
    end
endgenerate
// --------------------
...
assign o_val = g_frac_part_is_0 ? i_val[N-1:N_F]
