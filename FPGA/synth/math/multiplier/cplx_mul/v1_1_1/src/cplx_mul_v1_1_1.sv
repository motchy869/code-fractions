...
//!
//! Total cycle latency is ```2 + MULT_OUTPUT_STG_PIPELINE_DEPTH + ENABLE_ROUNDING_HALF_TO_EVEN```.
//!
...
//! ## changelog
//! ### [1.1.1] - 2024-11-27
//! - fix: missing reset for pipeline registers
//! ### [1.1.0] - 2024-09-24
//! - change: Renamed a parameter ```MULT_PIPELINE_DEPTH``` to ```MULT_OUTPUT_STG_PIPELINE_DEPTH```.
//! ### [1.0.2] - 2024-09-20
...
module cplx_mul_v1_1_1 #(
    ...
    parameter int unsigned MULT_OUTPUT_STG_PIPELINE_DEPTH = 2, //! multiplier output pipeline depth, must be greater than 0
...
// ---------- parameters ----------
localparam int unsigned CYCLE_LATENCY = 2 + MULT_OUTPUT_STG_PIPELINE_DEPTH + ENABLE_ROUNDING_HALF_TO_EVEN; //! cycle latency
...
generate
    if (BIT_SLICE_OFFSET_OUT + BIT_WIDTH_OUT - 1 > BIT_WIDTH_INTERM_PROD - 1) begin: gen_bit_slice_range_validation
        nonexistent_module_to_throw_a_custom_error_message_for_bit_slice_range_validation inst();
    end

    if (MULT_OUTPUT_STG_PIPELINE_DEPTH < 1) begin: gen_mult_OUTPUT_STG_pipeline_depth_validation
        nonexistent_module_to_throw_a_custom_error_message_for_mult_OUTPUT_STG_pipeline_depth_validation inst();
    end

    if (ENABLE_ROUNDING_HALF_TO_EVEN && BIT_SLICE_OFFSET_OUT == 0) begin: gen_rounding_param_validation
        nonexistent_module_to_throw_a_custom_error_message_for_rounding_param_validation inst();
    end
endgenerate
// --------------------
...
var logic signed [MULT_OUTPUT_STG_PIPELINE_DEPTH-1:0][BIT_WIDTH_INTERM_PROD-1:0] r_beta_0; //! Re(a) * (Re(b) + Im(b))
var logic signed [MULT_OUTPUT_STG_PIPELINE_DEPTH-1:0][BIT_WIDTH_INTERM_PROD-1:0] r_beta_1; //! (Re(a) + Im(a)) * Im(b)
var logic signed [MULT_OUTPUT_STG_PIPELINE_DEPTH-1:0][BIT_WIDTH_INTERM_PROD-1:0] r_beta_2; //! (Im(a) - Re(a)) * Re(b)
...
always_ff @(posedge i_clk) begin: blk_update_gammas
    if (i_sync_rst) begin
        ...
        r_gamma_0 <= r_beta_0[MULT_OUTPUT_STG_PIPELINE_DEPTH-1] - r_beta_1[MULT_OUTPUT_STG_PIPELINE_DEPTH-1];
        r_gamma_1 <= r_beta_0[MULT_OUTPUT_STG_PIPELINE_DEPTH-1] + r_beta_2[MULT_OUTPUT_STG_PIPELINE_DEPTH-1];
    end
end
