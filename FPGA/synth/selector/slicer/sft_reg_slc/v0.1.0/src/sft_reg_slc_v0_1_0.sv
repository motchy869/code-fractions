...
`ifndef SFT_REG_SLC_V0_1_0_SV_INCLUDED
`define SFT_REG_SLC_V0_1_0_SV_INCLUDED
...
// ---------- brief sketch ----------
// We use W_I - W_O + 1 shift registers.
// Each of them takes input vector and shifts content until the desired slice's first element is at the first position.
// At one clock positive edge, only one shift register takes new input from upstream-side, and all the other shift registers shift or maintain their content.
// --------------------
...
var logic [$clog2(N_SFT_REGS)-1:0] r_curr_io_sft_reg_idx; //! index of current shift register to take input and show output
// --------------------
...
assign o_out_vld = r_sft_reg[r_curr_io_sft_reg_idx].vld;
assign o_out_vct = r_sft_reg[r_curr_io_sft_reg_idx].vec[0+:W_O];
// --------------------

// ---------- blocks ----------
//! Updates the shift register index.
always_ff @(posedge i_clk) begin: blk_update_sft_reg_idx
    if (i_sync_rst) begin
        r_curr_io_sft_reg_idx <= '0;
    end else if (!i_freeze) begin
        if (r_curr_io_sft_reg_idx == $clog2(N_SFT_REGS)'(N_SFT_REGS-1)) begin
            r_curr_io_sft_reg_idx <= '0;
        end else begin
            r_curr_io_sft_reg_idx <= r_curr_io_sft_reg_idx + $clog2(N_SFT_REGS)'(1);
        end
    end
end

generate
    for (genvar i=0; i<N_SFT_REGS; ++i) begin: gen_update_sft_regs
        //! Updates the shift registers.
        always_ff @(posedge i_clk) begin: blk_update_sft_reg
            if (i_sync_rst) begin
                r_sft_reg[i] <= '{default:'0};
            end else if (!i_freeze) begin
                if (i == r_curr_io_sft_reg_idx) begin
                    r_sft_reg[i].vld <= 1'b1;
                    r_sft_reg[i].vec <= i_in_vct;
                    r_sft_reg[i].rem_sft <= i_slc_idx;
                end else begin
                    if (r_sft_reg[i].rem_sft > '0) begin
                        r_sft_reg[i].rem_sft <= r_sft_reg[i].rem_sft - $clog2(N_SFT_REGS)'(1);
                        r_sft_reg[i].vec[0+:W_I-1] <= r_sft_reg[i].vec[1+:W_I-1];
