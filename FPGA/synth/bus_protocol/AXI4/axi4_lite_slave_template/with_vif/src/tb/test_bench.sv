...
task automatic reg_check();
    ...
    for (int i=0; i<4; ++i) begin
        axi4_lite_access_t::axi4_lite_write(dut_vif, AXI4_LITE_ADDR_BIT_WIDTH'(i*4), write_data[i], '1, resp);
        @(posedge r_clk);
    end

    for (int i=0; i<4; ++i) begin
        axi4_lite_access_t::axi4_lite_read(dut_vif, AXI4_LITE_ADDR_BIT_WIDTH'(i*4), read_back_data, resp);
...
initial begin
    dut_vif = dut_if;
    axi4_lite_access_t::reset_mst_out_sigs(dut_vif, 1'b0);
    fork
        scenario();
    join_none
    #SIM_TIME_LIMIT_NS;
